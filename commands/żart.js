const zarty = [
    "Szedł sobie Chopin i bach",
    "Co to jest mieszanie pieprzu z dżemem?\n - Zebranie partyjne: jeden pieprzy, reszta drzemie...- Zebranie partyjne: jeden pieprzy, reszta drzemie...",
    "Przychodzi dwóch facetów obok Sejmu i słyszą jak wrzyscy drą\n się: Sto lat! Sto lat!\n Jeden do drugiego:\n - Ktoś tam chyba ma urodziny.\n - Nie. Wiek emerytalny ustalają. ",
    "Do domu sławnego polityka przychodzą dziennikarze.\n Drzwi otwiera im jego syn:\n - Twój tata jest w PSL, SLD czy AWS?\n - Mój tata jest teraz w WC.",
    "Lekarz i Turek kupują dwa takie same, ostatnie domy na osiedlu. Mieszkają obok siebie jakiś czas. W końcu Turek\n przychodzi do lekarza i mówi:\n - Mój dom jest więcej wart niż twój.\n - Dlaczego? Przecież są takie same.\n - Tak, ale ja mieszkam koło lekarza, a ty koło Turka.",
    "- Mamo, a ty chciałaś syna czy córkę?\n - Kochanie, ja po prostu chciałam zawiązać buty.",
    "Przychodzi facet do sklepu i mówi:\n - Poproszę okulary.\n - Do czego?\n - Do widzenia.\n - Do widzenia.",
    "- Tato, chcę się ożenić z Basią.\n - Przeproś. \n - Co?\n - Przeproś!\n - Ale za co?\n - Nieważne, przeproś.\n - Ale o co chodzi? \n - Przeproś!\n - Daj mi chociaż jakiś powód...\n - Po prostu przeproś.\n - No dobra. Przepraszam\n - OK, skoro jesteś w stanie przeprosić bez żadnego powodu to faktycznie jesteś gotowy na małżeństwo.",
    "Mówi kolega do kolegi:\n - Co to znaczy PKP? \n A drugi kolega na to: \n - Poczekaj, Kiedyś Przyjedzie.",
    "Kupiłem sobie wczoraj antyradar.\n Kupiłem sobie wczoraj antyradar.\n - Zorientowałem się, że ma homologację drogówki... \n - Jak to?! \n - ...gdy trzysta metrów przed ich zasadzką zaczął gadać: Szykuj pieniądze! Szykuj pieniądze!",
    "Mała dziewczynka rozmawiała z nauczycielem o wielorybach. Nauczyciel twierdził, że jest fizyczną niemożliwością, by wieloryb mógł połknąć człowieka Pomimo, że jest taki wielki, jego gardło jest zbyt wąskie. Dziewczynka upierała się, że przecież Jonasz został połknięty przez wieloryba. Zirytowany nauczyciel powtórzył, że to fizycznie niemożliwe.\n Na to dziewczynka:\n - Gdy będę w niebie zapytam o to Jonasza.\n - A jeśli Jonasz poszedł do piekła? - pyta złośliwie nauczyciel. \n - Wtedy pan go zapyta!",
    "Dlaczego blondynka kładzie pączka do mikrofalówki?\n Bo chce spalić kalorie.",
    "Co robi blondynka w szambie?\n Układa klocki.",
    "Dwie blondynki wybrały się na wycieczkę rowerową.\n Nagle jedna z nich zatrzymuje się i wypuszcza powietrze z kół.\n - Co robisz? - pyta zdziwiona koleżanka.\n - A nic, miałam za wysoko siodełko.\n Wtedy ta druga też zsiada z roweru i odwraca kierownicę oraz siodełko.\n - A ty, co robisz?\n - Wracam do domu. Nigdzie nie jadę z takim głupolem jak ty...",
    "Dwie blondynki oglądają mecz Brazylia vs Czechy,pierwsza poszla zrobic popcorn...druga krzyczy\n -GOOOL\n -kto strzelił?\n -Ronaldinho\n za chwile\n -jest drugi gol\n -jest drugi go\n -kto szczelił\n -jakis Replay",
    "Dlaczego blondynki nie wolno bić po głowie?\n - Bo jej echo zęby powybija.",
    "Dlaczego blondynka czołga się po super markecie?\n - Bo szuka niskich cen. ",
    "Rozmawiają dwie blondynki, nagle jedna mówi:\n - Kurcze, dzwoni mi w głowie\n - To odbierz, ja poczekam.",
   "Przychodzi Jasio do przedszkola i pyta się pani:\n - Czy dzieci mogą mieć dzieci?\n Pani odpowiada: \n - W żadnym wypadku!\n Jasio odwraca się do Małgosi i mówi:\n - I widzisz głupia!",
    "Nauczyciel pyta się Jasia: \n -Jakie kwiaty najbardziej lubisz? \n - Róże. \n - Proszę, napisz to na tablicy. \n - Chyba jednak wolę maki - mówi Jasio.",
    "Jasiu mówi do Taty:\n - Tato pójdziemy do cyrku\n - Nie mam czasu - mówi Tata czytając gazetę\n - Obiecałeś - wiesz podobno pokazują tam fajny numer naga \n kobieta jeździ na tygrysie. \n - No dobrze pójdziemy, Tata dawno nie widział tygrysa.",
    "Przychodzi Jasiu do sklepu i mówi: \n -poproszę fafkulce \n -przepraszamy, ale nie mamy czegoś takiego. \n Po czym Jasiu przychodzi po kilku dniach i mówi: \n -no poproszę fafkulce! \n -proszę wyjść nie mamy takiego czegoś. \n -no to poproszę fa w spreju.",
    "Jak się żegna informatyk? \n W imię Ojca \n i Syna \n i Ducha Świętego \n ENTER",
    "Dlaczego programista na umycie głowy zużywa całą butelkę\n szamponu?\n - Bo w instrukcji jest napisane:\n >>Nałożyć, spłukać. Czynność powtórzyć.<<",
    "Co mają w gębie informatycy?\n Blueteeth",
    "Panie doktorze, piec lat temu poradził mi pan znakomity\n sposób na\n reumatyzm - unikać wilgoci. Jestem wyleczony!\n - Znakomicie! A co panu teraz dolega?\n - Nic. Chciałbym zapytać, czy mogę się już wykąpać?",
    "Pani do Jasia:\n - Jasiu znasz Mickiewicza, Sienkiewicza i Makuszyńskiego?\n - A czy pani zna Łysego, Grubego lub Zenka?\n - Nie...\n - To co mnie pani straszy swoją bandą?!",
    "Dwie blondynki wygrały nagrodę.\n Nie wiedzą co z nią zrobić, aż w końcu Zuza mówi:\n - wiem, polecimy na słońce\n - co Ty spalimy się\n - przecierz polecimy w nocy.",
    "Bacę zaproszono w karnawale na bal przebierańców.\n Pytają go znajomi, za co się przebrał.\n - A za łoscypka.\n - A jak to, baco, za oscypka?\n - A siadne se w kąciku i bede śmierdział.",
    "Przychodzi facet do domu:\n - Kochanie, Regan został prezydentem!\n - A to Bierut już nie jest?"
];

module.exports.run = async (client, message, args) => {
const { RichEmbed } = require('discord.js');
  message.channel.send(new RichEmbed().setColor("RANDOM")
                       .addField("Oto losowy żart:", zarty[Math.floor(Math.random()*zarty.length)]))
}

module.exports.help = {
  name: "żart"
}