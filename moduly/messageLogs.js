module.exports = async (client) => {
    ...
    if(message.channel.type === "dm") return;
    let logChannel = message.guild.channels.find("name", db.fetch(message.guild.id + ".logMessage") || "fdsdsffdsdsfdsffdsdsf"); //Definiowanie kanału
    ...
    if(oldMessage.author.bot) return;
      if(oldMessage.channel.type === "dm") return;
      let logChannel = newMessage.guild.channels.find("name", db.fetch(newMessage.guild.id + ".logMessage") || "fdsdsffdsdsfdsffdsdsf"); //Definiowanie kanału
      if(!logChannel) return;
