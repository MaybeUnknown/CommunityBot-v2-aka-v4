module.exports = async (client) => {
    ...
    cmd.get(`
       ...
       function(err, data, stderr){
         ...
         client.channels.get("540936381485154304").send(new RichEmbed()
                                                            .setColor(3447003)
                                                            .setAuthor(req.user.username, `https://cdn.discordapp.com/avatars/494017032283619329/${req.user.avatar}?width=676&height=676`)
  ...
  app.post("/dashboard/:gid/ustaw", checkAuth, function(req, res) {
      ...
      if(req.body.idYt) {
        db.set(req.params.gid + ".idYt", req.body.idYt);
        //db.set(req.body.idYt + ".id", req.body.ytCh);
      }
      if(req.body.idTr) {
        db.set(req.params.gid + ".idTr", req.body.idTr);
      }
      if(req.body.ytCh) {
        db.set(req.params.gid + ".ytCh", req.body.ytCh);
      }
      if(req.body.notCh) {
          req.body.notCh.forEach(id => {
              db.push(req.params.gid + ".notCh", id);
          })
      }
  ...
  app.post("/profil/ustaw", checkAuth, function(req, res) {
    ...
    if(req.body.tiktok) {
      ...
      if(req.body.youtube) {
        db.set(req.user.id + ".youtube", req.body.youtube)
      } 
      else {
        db.set(req.user.id + ".youtube", "null")
      }
