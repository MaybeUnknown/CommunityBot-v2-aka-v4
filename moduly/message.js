...
message.channel.send("OK zweryfikowano " + member).then(m => setInterval(function(){ m.delete() }, 5000));
    ...
    if (message.channel.id === (db.fetch(message.guild.id + "ping") || null)) {
            if(message.content !== "ping" && message.content !== "pong" && message.content !== "Pong" && message.content !== "Ping")  return message.delete();
            ...
            db.set(db.fetch(message.guild.id + "ping") + ".lastuser", message.author.id);
            db.set(db.fetch(message.guild.id + "ping") + ".lastvalue", message.content);
            if(message.content === "ping" || message.content == "Ping") return message.channel.setTopic("Następne jest pong");
            if(message.content === "pong" || message.content == "Pong") return message.channel.setTopic("Następne jest ping");
    ...
    let tag = db.fetch(message.guild.id+".tags."+command.slice(prefix.length));
    if(tag&&tag!=="null") return message.channel.send(tag);
    ...
    if (command.startsWith(prefix)) { //Jeśli wiadomość jest z prefixem
      if (db.fetch(message.author.id + ".black") === true) { //Blacklista
        ...
        if (cmd) {
            ...
            try {
                await db.set(message.author.id  + "k" + cmd,  Date.now() + cmd.help.timeout * 1000);
            } catch(e) {
                console.log(e);
      ...
      client.channels.get("602790673682268183").send(new RichEmbed()// logi użytych komend 
                                                     .setColor("RANDOM")
                                                     .setAuthor(message.author.username, message.author.avatarURL)
                                                     .addField(`Komenda:`, cmd.help.name)
                                                     .addField(`Serwer: `, message.guild.name)
                                                     .addField(`Kanał:`, message.channel.name)
                                                     .addField(`Argument:`, args.join(" ")||'_Brak_')
